...
public class ItemModDye extends Item {
    public static String[] dyeColors = {"crimson","maroon"};
    ...
    public static Item createDyes(String name, int id) {
        ...
        int[] mainCoords = TextureHelper.registerItemTexture(GoocraftBTA.MOD_ID, dyeColors[0]+"_dye.png");
        //save the icon index for later
        dyeIndex = Item.iconCoordToIndex(mainCoords[0],mainCoords[1]);
        for (int i=1;i<dyeColors.length;i++) {
            TextureHelper.registerItemTexture(GoocraftBTA.MOD_ID, dyeColors[i]+"_dye.png");
        }
